...
| [ ] 3b.6f3c | Record a pattern so fine that no offered resolution can reach the 3 px minimum | The dialog says **no recording size can get them there** and asks for a **coarser pattern or a shorter standoff**, then names the **closest size this camera has**. **Change resolution** still goes to that size — it is the direction to move in even though it does not reach the band — and must never be worded as though it fixes the problem |
